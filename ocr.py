...
def extract_text(image_path: str) -> str:
    ...
    # 2. PRE-PROCESSING 
    # Perbesar gambar 2x lipat agar hurufnya lebih jelas dibaca Tesseract
    img_resized = cv2.resize(img, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    
    # Ubah ke hitam putih (Grayscale)
    gray = cv2.cvtColor(img_resized, cv2.COLOR_BGR2GRAY)
    
    # Otsu's Thresholding (Lebih pintar memisahkan tinta hitam dan kertas putih)
